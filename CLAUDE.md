...
- **엑셀 탭은 `SHOW_IO_TAB` 상수로 껐다 켠다**(현재 `false`). 코드(`pageExcel`·`importPane`·
  `collectPane`·`csvRows`)는 **전부 살아 있고** 되살리기는 한 줄이다.
  ★ **🧩 제출 취합이 이 탭 안의 서브탭이라 함께 숨겨진다** — 취합을 다시 쓰려면 `true` 로 켤 것.
  **개인 배포판의 `내보내기 / 불러오기` 탭은 이 상수와 무관하게 항상 보인다** — 제출 파일을
