...
- **`submissions[]` — 과별 제출값 원본.** 대표값(노드 본체)은 '첫 제출자 승리'라 3개 과가 같은
  ...
  - 취합은 `(과,이름)` 1명 = `count:1` 레코드 1건만 만들고, **같은 과·같은 값의 합산은
    `schema._norm_submissions` 가 한다**. 합산 규칙을 두 곳에 두지 않으려는 것이고, 덕분에
    `sum(rec.count) == submit_count` 가 자동으로 성립한다.
...
| 화면 | 무엇을 |
|---|---|
| 상세 패널 `🏢 과별 제출값` | 그 lv6 을 낸 **과마다 한 줄** — 인원·1회 소요·주기·부하·자동화·기술 (읽기전용) |
| 📊 요약 탭 필터바 | 부서/과를 고르면 우측 목록이 좁혀지고, 각 행의 부하·자동화가 **그 과의 제출값**으로 바뀐다 |
| 엑셀 `제출상세` 시트 | 1행 = (업무 × 과). **역수입하지 않는다** — 정본은 트리 JSON 의 `submissions` |
| 카드·전체보기의 `👥 N명` 배지 | **마우스 오버**하면 과별 제출 내역(`submitTip`). 컬럼·전체보기 양쪽에 단다 |

- **배지 툴팁에 사람 이름은 없다.** `collect_jsons` 가 이름을 인원수 세는 데만 쓰고 버리기 때문이다
  (공통규칙 7 최소수집 — `_submission_record` 가 과만 담는다). 이름이 필요하면 취합 미리보기의
  '파일별 상태' 를 봐야 하고, 그것도 **스캔 시점에만** 존재한다. 이름을 트리에 저장하지 말 것.
...
계층 편집 상단의 `편집 범위` 에서 부서/과를 고르면 **그 범위 밖 업무는 보이되 잠긴다**.
정본 하나를 여러 부서가 함께 다듬을 때 남의 업무를 실수로 고치는 것을 막는 장치다.

- **숨기지 않고 잠그는 이유**: 상위 분류(lv3~lv5)는 소속이 없어 숨기면 내 업무가 어디 매달렸는지를
  잃고, 다른 과의 중복 제출도 안 보이게 된다.
...
- **새 업무 추가는 과까지 골랐을 때만** 허용한다. 부서만 고르면 새 lv6 에 줄 소속이 없어
  만들자마자 잠기기 때문이다. 과를 고른 상태의 새 lv6·lv7 은 그 과 소속으로 만들어진다.
- 요약 탭의 보기 필터(`S.fGroup`/`S.fDept`)와 **상태를 공유하지 않는다**(`S.eGroup`/`S.eDept`).
  한쪽은 "무엇을 볼까", 이쪽은 "무엇을 고칠 수 있나"라 의미가 다르다. 부서·과 목록만 `deptSelects` 로 공유한다.
