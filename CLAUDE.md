...
| `save_as` | JS→PY | 현재 화면 트리를 **이름 붙여 보관** → `store.save_named` (`saves/<이름>.json`). 같은 이름은 `overwrite` 없이는 거부 |
| `saves_load` / `saves_delete` | JS→PY | 보관본을 **화면에만** 불러오기(정본은 [저장]에서) / 삭제 |
...
`이력·복원` 탭 아래쪽. "2026-08 1차 취합" 처럼 **사람이 의미를 붙여 남기는 기준점**이다.
