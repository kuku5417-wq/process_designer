...
- **상단바는 한 줄에 들어가야 한다.** 환경(사내망/사외망) 표기를 뺐고, 저장·다시 읽기·보기 토글은
  **아이콘만**(뜻은 `title` 로 남긴다), 적용률 pill 은 2줄(`.pill.two`)이다.
  ★ **한 줄 한계는 1248px** (2026-08-25 실측: 1250 ok / 1220 접힘). `layout="wide"` +
  `.block-container{max-width:100%}` 라 **컨테이너 폭 ≈ 창 폭**이다 — 1366 에서 여유 118px,
  1920 에서 672px, 1280 에서 32px.
  탭이 6개가 되면서 한 번 두 줄로 넘어갔고(내용 합 ~1380px), **탭 라벨을 한 낱말로 줄여**
  되돌렸다(탭 묶음 531→366px). 줄인 뜻은 `TABS()` 의 `tip` → `title` 로 살려 둔다.
  더 줄여야 하면 `pill two`(166px) · `revcap`(39px) · `brand`(157px) 순으로 후보가 있다.
  `.top` 의 `flex-wrap: wrap` 은 그래도 유지한다 — 더 좁아지면 잘리는 것보다 접히는 게 낫다.
  요소를 더할 땐 **이 여유부터 확인**할 것.
...
- **동명 카드 드래그 병합**: 카드를 **이름이 같은 같은 레벨 형제** 위에 떨어뜨리면 한 노드로 합친다.
  하위도 이름으로 짝지어 **재귀 병합**하고, 짝이 없는 하위는 받는 쪽 아래로 옮긴다.
  취합·붙여넣기가 이름 경로로 병합하므로 동명 형제가 남으면 집계가 갈리는데, 예전에는 경고 배너만
  띄우고 합칠 수단이 없어 하위를 손으로 하나씩 옮겨야 했다.
  - 드롭존은 `.cmerge` — **드래그 중인 카드와 이름이 같은 형제에만** `armMergeZones` 가 `.on` 을 단다.
    전부 열어두면 서로 다른 업무가 실수로 합쳐진다. 그룹은 `L{lv}` — **같은 레벨**을 받는 유일한
    드롭존이다(`.cdrop` 은 `L{lv+1}` = 상위 바꾸기).
  - ★ **값은 받는 쪽(드롭 대상)이 정본**이고 끌어온 쪽은 빈 칸만 채운다. 목록형(`tech`·`depts`·
    `ship_types`…)은 합집합, 객체배열(`linked_systems`·`events`·`submissions`)은 JSON 서명으로 중복
    제거 후 이어붙이고, `future_years` 는 **늦은 해**가 이긴다(`rollupFutureYears` 와 같은 규칙).
    스칼라가 양쪽 다 값이 있고 다르면 **받는 쪽을 남기고 몇 칸을 버렸는지 알린다** — 조용히
    덮어쓰면 어느 값이 맞았는지 아무도 모르게 된다.
  - 되돌릴 수 없으므로 **확인 모달**을 거친다(자손 삭제와 같은 등급). 모달 본문은 `esc()` 로
    그대로 나가므로 **마크다운 강조를 쓰면 별표가 그냥 보인다.**
  - ★ 사라진 노드 id 를 **`S.deleted` 에 넣어야 한다** — 부분 저장은 만진 노드만 얹으므로
    안 넣으면 병합한 것이 다음 저장에 되살아난다(`doDel` 과 같은 이유).
  - 편집 범위 게이트를 **`actMergeInto` 에서도 본다** — 드롭존 표시는 화면일 뿐이라
    `actMoveMany` 와 같은 이유로 판정을 여기서 한 번 더 한다.
  - 다중선택 드롭에 이름이 다른 카드가 섞이면 그것만 빼고 **몇 개를 건너뛰는지 모달에 적는다**.
  - **일괄 병합**: 중복 경고 배너의 `⧉ 전부 합치기`(`actMergeAll`) — 남아 있는 동명 묶음을 한 번에
    정리한다. 기존 데이터를 처음 이 앱에 얹을 때 하나씩 드래그하지 않아도 되게 하는 통로다.
    - ★ **얕은 레벨부터 한 라운드씩 돌고 매 라운드 다시 스캔한다.** lv4 를 합치면 그 아래 lv5·lv6 이
      `mergePair` 로 함께 정리되므로, 먼저 훑어둔 목록을 그대로 쓰면 이미 사라진 노드를 가리킨다.
    - ★ **합쳐진 노드를 `S.nodes` 에서 빼고 나서** 다시 스캔해야 한다 — 안 빼면 같은 그룹이 계속
      잡혀 **무한 루프**가 된다. 그래도 라운드 상한(60)을 둔다.
    - 편집 범위가 걸려 있으면 **전원이 내 범위 안인 묶음만** 합치고, 제외한 묶음 수를 모달에 적는다.
    - ★ 확인 모달 본문은 `esc()` 로 나간다 — **마크다운 강조를 쓰면 별표가 그대로 보인다**(두 번 겪었다).
    - **빈 가지도 함께 지운다** — 세부업무(lv6)에 닿지 않는 lv3~lv5. 판정은 `emptyBranches()` 이고
      `descLoadNodes`(편집 범위 게이트가 쓰는 자손 lv6 목록)를 그대로 쓴다 — 기준이 갈리지 않게.
      `excel_io.prune_empty_branches`(취합)와 **같은 대상**이라 두 경로의 결과가 일치한다.
      · 목록은 **맨 위 가지만** 센다(자손은 함께 지워지므로 두 번 세지 않는다). 모달은 그 수를,
        flash 는 **실제 지운 노드 수**를 말하므로 둘이 다를 수 있다 — 모달에 "그 아래 분류도 함께" 를 적는다.
      · ★ **병합을 끝낸 뒤에 다시 계산한다.** 합치는 과정에서 하위가 옮겨 다니므로 미리 잡아둔
        목록은 낡는다(빈 줄 알았던 가지에 lv6 이 들어와 있을 수 있다).
      · 편집 범위 밖 빈 가지는 건드리지 않는다.
    - 중복이 없고 **빈 가지만 있을 때도 배너가 뜬다**(버튼 라벨이 `🧹 빈 가지 정리` 로 바뀐다) —
      정리 통로가 이 버튼 하나뿐이라 닿을 수 없으면 안 된다.
- **같은 이름 형제 중복 가드**: `dupSiblings`(=`schema.duplicate_siblings`, **lv4+** 형제 동명, 하위 미포함,
  `validate` 아님) 이 있으면 상단에 상시 경고 배너(+`⧉ 전부 합치기` 버튼), **개인 배포판 '제출 파일 내보내기'는 차단**
